...
module.exports = {
  ...
  run: async (client) => {
    const embed = new EmbedBuilder()
      .setDescription(`*Le bot a démarré* <t:${Math.floor(Date.now() / 1000)}:R>.\n\n**Développeurs :**\n [\`${(await client.users.fetch("355851488372719618")).username}\`](${client.support}) \`&\` [\`${(await client.users.fetch("648236998657835047")).username}\`](${client.support}) \`&\` [\`${(await client.users.fetch("1095592017121251400")).username}\`](${client.support})\n\n**Besoin d'aide ? Rejoins notre support :** [\`[Clique ici]\`](${client.support})`)
